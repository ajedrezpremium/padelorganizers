...
import { COURT_STATUS, getInitialDemoTournamentData, finishMatch } from '../services/padelEngine';
...
const I18N = {
  ...
  fr: {
    ...
    statWon: 'J',
    active: 'Actif',
    formatAmericano: 'Format américain · Point d\'or activé',
  },
  pt: {
    title: 'Painel de controlo — Torneio Dashboard',
    subtitle: 'Open Vigo Pro 2026 — Clube Vigo Bouzas',
    statCourts: 'Pistas',
    statMatches: 'Jogos',
...
export default function PadelDashboard({ lang = 'es' }) {
  ...
  const [data, setData] = useState(() => getInitialDemoTournamentData());
  ...
  const handleAssignCourt = (courtId, matchId) => {
    ...
    setData({ ...data, courts: updatedCourts, matches: updatedMatches });
  };

  const handleFinishMatch = (matchId, g1, g2) => {
    setData(prev => finishMatch(prev, matchId, g1, g2));
  };
    ...
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        <div style={statCardStyle}>
          ...
          <div style={statLabelStyle}>🏟️ {T.statCourts}</div>
        </div>
        <div style={statCardStyle}>
      ...
      <div style={{ background: '#0a1a17', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 16px', fontSize: '12px', color: '#64748b', fontWeight: 700, textTransform: 'uppercase', borderBottom: '1px solid rgba(16,185,129,0.2)' }}>
          <span>{T.statRank}</span>
          <span style={{ flex: 2, paddingLeft: '8px' }}>{T.statPair}</span>
          <span style={{ flex: 1, textAlign: 'center' }}>🎯 {T.statPoints}</span>
          <span style={{ flex: 1, textAlign: 'center' }}>📈 {T.statDiff}</span>
          <span style={{ width: '40px', textAlign: 'center' }}>{T.statWon}</span>
        </div>
        {data.pairs.map((p, i) => (
